...
If !DisableDermCoding
	...
	Menu, SettingsMenu, Add, Disable Derm Coding Features, DisableDermCodingFeatures
	...
	If DisableDermCoding
		{
			Menu, SettingsMenu, Check, Disable Derm Coding Features
			Gosub, DermHotKeysOff
		}
	else
		{
			Menu, SettingsMenu, UnCheck, Disable Derm Coding Features
			Gosub, DermHotKeysOn
		}
...
ButtonOK:  ;Automation
{
	...
	If (!DisableDermCoding AND UndoEnabled AND !DataEntered)
		{
	...
	If ((DisableDermCoding OR DataEntered) AND UndoEnabled)
		{
			Gosub, F8
			If SaveError
			{
...
DisableDermCodingFeatures:  	;Paper replacer
{
	Menu, SettingsMenu, ToggleCheck, Disable Derm Coding Features
	If DisableDermCoding
		{
			DisableDermCoding := 0
			Gosub, DermHotKeysOn
	...
	IniWrite, %DisableDermCoding%,  %A_MyDocuments%\CarisCodeRocket.ini, Window Positions, DisableDermCoding
...
^!s::  ;Batch Signout		;Automation
{
	...
	if (SignOutCount>0)
	{
		...
		if((A_PriorKey="F3" OR A_PriorKey="ScrollLock") and A_TimeIdleKeyboard<1500)
			{
...
^!r::
{
Loop, Read, S:\CodeRocket\bin\AHK Files\PaperReplacer\version.txt
	{
	CurrentVersion="s:\CodeRocket\bin\AHK Files\PaperReplacer\%A_LoopReadLine%"
	Break
	}

Run, S:\CodeRocket\bin\AHK Files\PaperReplacer\Autohotkey U32.exe  %CurrentVersion%
ExitApp
}
...
ReadIniValues()
{
	...
	IniRead, DisableDermCoding, %A_MyDocuments%\CarisCodeRocket.ini, Window Positions, DisableDermCoding
