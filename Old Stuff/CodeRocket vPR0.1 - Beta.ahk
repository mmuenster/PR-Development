...
11ButtonSave:
{
	...
	cbB9NeviMargins:= CP.document.getElementById("cbB9NeviMargins").checked
	cbMildDNMargins:= CP.document.getElementById("cbMildDNMargins").checked
	cbModDNMargins:= CP.document.getElementById("cbModDNMargins").checked
	cbSevDNMargins:= CP.document.getElementById("cbSevDNMargins").checked
	cbMelanomaMargins:= CP.document.getElementById("cbMelanomaMargins").checked

	cbNMSCMargins:= CP.document.getElementById("cbNMSCMargins").checked
	cbAKMargins:= CP.document.getElementById("cbAKMargins").checked

	cbExcisionsMargins:= CP.document.getElementById("cbExcisionsMargins").checked
	cbAllNeoplasmMargins:= CP.document.getElementById("cbAllNeoplasmMargins").checked

	cbB9NeviRecomendations:= CP.document.getElementById("cbB9NeviRecomendations").checked
	cbMildDNRecomendations:= CP.document.getElementById("cbMildDNRecomendations").checked
	cbModDNRecomendations:= CP.document.getElementById("cbModDNRecomendations").checked
	cbSevDNRecomendations:= CP.document.getElementById("cbSevDNRecomendations").checked
	cbMelanomaRecomendations:= CP.document.getElementById("cbMelanomaRecomendations").checked

	cbNMSCRecomendations:= CP.document.getElementById("cbNMSCRecomendations").checked
	cbAKRecomendations:= CP.document.getElementById("cbAKRecomendations").checked

	cbExcisionsRecomendations:= CP.document.getElementById("cbExcisionsRecomendations").checked
	cbAllNeoplasmRecomendations:= CP.document.getElementById("cbAllNeoplasmRecomendations").checked

	cbUseMicros:= CP.document.getElementById("cbUseMicros").checked
	cbUsePhotos:= CP.document.getElementById("cbUsePhotos").checked

	editDoNotUseList := CP.document.getElementById("editDoNotUseList").value
	
	dataAsJson={ "preferenceBase":"%preferenceBase%", "B9NeviMargins":"%cbB9NeviMargins%", "MildDNMargins":"%cbMildDNMargins%","ModDNMargins":"%cbModDNMargins%", "SevDNMargins":"%cbSevDNMargins%", "MelanomaMargins":"%cbMelanomaMargins%", "NMSCMargins":"%cbNMSCMargins%", "AKMargins":"%cbAKMargins%", "ExcisionsMargins":"%cbExcisionsMargins%", "AllNeoplasmMargins":"%cbAllNeoplasmMargins%", "UsePhotos":"%cbUsePhotos%", "UseMicros":"%cbUseMicros%", "DoNotUseList":"%editDoNotUseList%" }
	
	FileDelete, S:\Coderocket\ClientPreferences\%PhysicianWinSurgeId1%-%ClientID%.txt
	FileAppend, %dataAsJson%, S:\Coderocket\ClientPreferences\%PhysicianWinSurgeId1%-%ClientID%.txt
	tempVar := Json_Load(dataAsJson)
	Msgbox, % tempVar.B9NeviMargins
	Gui 11:Destroy
	return
}
		...
		If ErrorLevel
			return
				...
				if (i=0)
				{
					...
					tempVar += Instr(UseMargins, "margins-all-nevi") AND (Instr(SelDiagnosis, "nevus") OR Instr(SelDiagnosis, "melanocy"))  ;For All nevi
					if (tempVar)
					{
						SoundBeep 
						SoundBeep
					...
					tempVar := Instr(UseMargins, "none")  ;Warn if they have requested no margins
					tempVar += Instr(UseMargins, "no-margins-nmsc") AND (Instr(SelDiagnosis, "carcinoma") OR Instr(SelDiagnosis,"actinic keratosis")) ;warn if they have requested no margins on NMSC
					if (tempVar)
						{
							SoundBeep
							SoundBeep
...
LoadSmartSubstitutions()  ;PaperReplacer
{
	...
	IfExist, %A_MyDocuments%\MySmartSubstitutions.csv
	{
		...
		Loop, read, %A_MyDocuments%\MySmartSubstitutions.csv
		{
			...
			smartSubs[A_Index] := { "searchText":sSub1, "replacementText":sSub2 }
		}
	}
